...
import { _saveConfig, SaveConfigProps } from './actions';
...
const DesignConfigurator = ({ configId, imageUrl, imageDimensions }: Props) => {
  ...
  const { mutate: saveConfig, isPending } = useMutation({
    ...
    mutationFn: async (data: SaveConfigProps) => {
      await Promise.all([saveConfiguration(), _saveConfig(data)]);
    },
    onSuccess: () => {
                    ...
                    <DropdownMenuContent>
                      {MODELS.options.map((model) => (
                        <DropdownMenuItem
                          key={model.label}
                          className={cn(
                            'flex text-sm gap-1 items-center p-1.5 cursor-defult hover:bg-zinc-100 cursor-pointer',
                            {
